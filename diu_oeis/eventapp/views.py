...
def delete_event(request, event_id):
    ...
    club_ec = Club_Ec.objects.get(ec=request.user)
    club = Clubs.objects.get(pk=club_ec.club_id)
    if event.created_by != club:
        return redirect('hostedevent')  # Redirect if user is not authorized to delete the event
    if request.method == 'POST':
        event.delete()
        messages.success(request, 'Event successfully deleted')
        return redirect('hostedevent')
    context = {'event': event}
    return render(request, 'club/deleteevent.html', context)
